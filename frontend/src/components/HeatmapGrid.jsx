...
export default function HeatmapGrid({ members, reasons, columns, mode, hourFrom = 0, hourTo = 23, minuteStep = 60, heatColors, focusMode = false, compareCount = 0 }) {
  ...
  const [hover, setHover] = useState(null); // {ci, slotIdx}
  ...
  // Compute free count per (col, slot)
  const grid = useMemo(() => {
  ...
  return (
    <div className="neo-card p-4 sm:p-6" data-testid="heatmap">
            ...
            <Fragment key={`row-${hour}-${minute}`}>
              ...
              {columns.map((c, ci) => {
                ...
                if (focusMode && total === 1) {
                  ...
                  bg = heatColor(cell.free.length, total, heatColors);
                }
                ...
                return (
                  <div
                    ...
                    className="heat-cell rounded-md"
                    ...
                    title={`${cell.free.length}/${total} free`}
                  >
                    {isHover && (
                      <div
                        className={`absolute ${tipPos} top-full mt-2 z-30 w-72 neo-card p-4 text-xs pointer-events-none`}
